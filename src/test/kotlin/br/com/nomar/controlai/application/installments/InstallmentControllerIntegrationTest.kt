...
class InstallmentControllerIntegrationTest {
    ...
    @Test
    fun `POST preview with a credit card paymentMethodId should use the plain calendar month`() {
        ...
        jdbcTemplate.update(
            "INSERT INTO payment_methods (name, type, holder_id, group_id) VALUES ('Preview Card', 'CREDIT_CARD', ?, 1)",
            holderId,
        )
        ...
        try {
            ...
            // installments (see BudgetPeriodResolver.resolveInstallmentDueDate): no closing-day
            // rollover, the first installment stays in the purchase's own calendar month.
            mockMvc.perform(
    ...
    fun `POST reconcile should recalculate due dates after a card period was manually corrected`() {
        ...
        jdbcTemplate.update(
            "INSERT INTO payment_methods (name, type, holder_id, group_id) VALUES ('LATAM Pass', 'CREDIT_CARD', ?, 1)",
            holderId,
        )
        val paymentMethodId = jdbcTemplate.queryForObject(
        ...
        // Raw, uncorrected calendar-month period as auto-generated by EnsureFutureBudgetProvider.
        jdbcTemplate.update(
